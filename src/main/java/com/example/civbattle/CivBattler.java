    ...
    public void start(Stage stage) {
        ...
        dialog.setResultConverter(button -> {
            if (button == okButtonType) {
                try {
                    int rows = rowsField.getText().isEmpty() ? 24 : Math.min(24, Integer.parseInt(rowsField.getText().trim()));
                    int cols = colsField.getText().isEmpty() ? 24 : Math.min(24, Integer.parseInt(colsField.getText().trim()));
                    int civs = civField.getText().isEmpty() ? 4 : Math.max(8, Integer.parseInt(civField.getText().trim()));
